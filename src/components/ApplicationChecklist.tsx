...
import { getApplication, getApplicationChecklist } from '../data/api';
...
const ApplicationChecklist: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        if (id) {
          ...
          const checklistData = await getApplicationChecklist(id);
          setCategories(checklistData);
        }
      } catch (err) {
        console.error("Error fetching application checklist:", err);
  ...
  // Handle requesting validation for the entire application checklist
  const handleFullValidation = async () => {
    ...
    try {
      ...
      const response = await requestBatchValidation({
        ...
        evidence_context: `Full application validation for ${application?.name || 'application'}`
      });
    ...
    <div>
      <div className="bg-white shadow rounded-lg p-4 mb-5">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-gray-900 mb-1">{application?.name} Checklist</h2>
            <p className="text-gray-600">{application?.description || 'No description available'}</p>
          </div>
          <button
      ...
      {categories.length === 0 ? (
        <div className="bg-white shadow rounded-lg p-4 text-center text-gray-500">
          No checklist categories found for this application.
        </div>
      ) : (
